 ...
 *********************************************************************/
#include <sys/types.h>    // for u_char, u_short, u_int
#include <netinet/in.h>   // for IPPROTO_*, ntohs, etc.
#include <netinet/ip.h>   // for struct ip
#include <netinet/tcp.h>  // for struct tcphdr
#include <netinet/udp.h>  // for struct udphdr
#include <net/ethernet.h> // for struct ether_header, ETHERTYPE_IP
#include <pcap.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int find_bucket(const flow_key_t *key, uint32_t hash, int *found, int action_flag)
{
    ...
    if (action_flag == 0) {
        ...
        if (!compare_key(&e->key, key)) {
            *found = 1;       // same flow
            return idx;
        }
        // occupied by different flow → collision, ignore
        return -1;
        ...
        if (compare_key(&e->key, key)) {
            *found = -1;
            ...
            int flow_remover = rand() % 2;  // 0 or 1

            if (flow_remover == 0) {
                if (e->count > 0) {
                    dump_and_clear(e);  // or manually flush data here
                } else {
                    memset(e, 0, sizeof(*e)); // just remove empty flow
                }
                return idx;
            } else {
                return -1;
...
static void track_packet(const struct timeval *tv,
                         ...
                         u_short ip_len, int action_flag)
{
    /* --- build canonical key & bucket --- */
    flow_key_t key = make_key(sip, dip, sport, dport, proto);

    /* hash the binary struct instead of stringifying */
    uint32_t h = hash_key(&key);
    int found;
    int idx = find_bucket(&key, h, &found, action_flag);
    if (idx < 0) { 
...
static int parse_and_track(const struct pcap_pkthdr *h, const u_char *pkt, int action_flag)
{
    ...
    track_packet(&h->ts, sip, dip, sport, dport, proto, tcp_syn, tcp_fin, ip_length, action_flag);
    return 1;
}
...
{   
    ...
    while ((rc = pcap_next_ex(pc, &h, &pkt)) >= 0) {
        ...
        parse_and_track(h, pkt, action_flag);
        if (++pkt_counter % 1500 == 0) {
            flush_idle_udp(&h->ts);  // only every 1500 packets
        }
        /*flush_idle_udp(&h->ts);*/
